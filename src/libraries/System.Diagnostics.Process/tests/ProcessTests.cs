...
namespace System.Diagnostics.Tests
{
    public partial class ProcessTests : ProcessTestBase
    {
        ...
        public void ResolveExecutablePath_PathWithDirectoryComponent_ResolvesWithoutSearchingPath()
        {
            ...
            // A path that includes a directory component resolves directly to the file, returned as provided.
            Assert.Equal(scriptFile, Process.ResolveExecutablePath(scriptFile), ignoreCase: OperatingSystem.IsWindows());
        ...
        [ConditionalFact(typeof(RemoteExecutor), nameof(RemoteExecutor.IsSupported))]
        public void ResolveExecutablePath_DirectoryComponent_ReturnsPathAsResolvedWithoutNormalizing()
        {
            string wd = Path.Combine(TestDirectory, "NoNormalize");
            Directory.CreateDirectory(Path.Combine(wd, "sub"));
            ...
            {
                // Place the executable in ./sub relative to the working directory.
                WriteScriptFile(Path.Combine(Directory.GetCurrentDirectory(), "sub"), "tool", returnValue: 0);

                // A non-canonical relative path with a redundant "." segment must be returned as
                // provided; the resolver does not normalize it (it never calls Path.GetFullPath).
                string sep = Path.DirectorySeparatorChar.ToString();
                string input = "." + sep + "sub" + sep + "tool";
                string? resolved = Process.ResolveExecutablePath(input);
                Assert.NotNull(resolved);

                // On Windows a PATHEXT extension is appended; on Unix the path is returned verbatim.
                string expected = OperatingSystem.IsWindows() ? input + ".bat" : input;
